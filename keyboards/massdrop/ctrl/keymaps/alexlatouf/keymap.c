...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case L_PTN:
            ...
            if (led_animation_id == 0) { // custom heating up animation
                led_instructions[0].flags = LED_FLAG_USE_RGB;
                led_instructions[0].r = 255;
                led_instructions[0].g = 0;
                led_instructions[0].b = 0;
            } else {
                led_instructions[0].flags = LED_FLAG_USE_ROTATE_PATTERN;
            }
            return false;
        case L_PTP:
            if (record->event.pressed) {
...
led_instruction_t led_instructions[] = {
    ...
    //All LEDs use the user's selected pattern (this is the factory default)
     { .flags = LED_FLAG_USE_RGB, .r = 255, .g = 0, .b = 0},
